...
| **Perio charting** | Six probing depths per tooth, with bleeding on probing. Depths of 4mm and 5mm+ are highlighted. A summary shows bleeding %, pocket counts and the change since the last exam. Exam history is kept. **Voice perio**: say the readings and the chart walks the probing path; “bleeding”, “pus”, “plaque”, “bleeding all”, “tooth 14”, “lingual”, “margins” (with “minus 2” for overgrowth), “skip”, “back”, “next tooth” and “missing” work hands-free, heard by the office's medical speech service when one is set up. |
...
| **Prescriptions & e-prescribing** | One-click favourites for common dental drugs, a preferred pharmacy for each patient, and printed or electronic prescriptions. E-prescribing runs through **DoseSpot** (Surescripts-certified, DEA-audited for controlled substances), opened from the chart with single sign-on. Only the prescriber can sign and send an electronic prescription. Controlled substances follow the DEA rules, with the schedule taken from the drug itself rather than the form: a DEA number, signed by the prescriber's own login with a fresh two-factor code that can't be reused, and no refills on Schedule II. Allergy screening catches drug classes, not just exact names: a codeine allergy flags hydrocodone, and a penicillin allergy flags cephalexin. Before a controlled substance the prescriber checks the state **prescription monitoring program (PDMP)** in one key (or records what the state website showed, or why it was skipped); the check is kept on the prescription and a failed check is a Needs attention item. |
| **Compliance log** | Manage → Compliance log. **Complaints and incidents** (anyone records one with N; the follow-up becomes a task for its owner; resolved or voided with a reason, never deleted; the manager sees the whole log, a 12-month report and a CSV). **Staff exposures and sharps injuries** (OSHA): who, how, device, source patient, the dated post-exposure checklist, and the sharps injury log as a CSV without names — a separate permission (`compliance:exposures`), as they are confidential employee medical records. **HIPAA accounting of disclosures**: each disclosure outside treatment, payment and operations (to whom, what, why, when, who recorded it); sending a record to someone else from the chart records it automatically; the patient's six-year accounting prints as a PDF or CSV (`compliance:manage`). |
| **Letters & mailing labels** | Letter templates with merge fields filled from the chart (name, address, dentist, next visit, balance, planned treatment…): pick one with 1–9, Enter prints, M emails; each letter is filed in the patient's documents (Letters). A field the chart has nothing for, or a "[blank]" left in the wording, blocks printing and sending. The same letter for a report's results in one PDF. **Mailing labels** (Avery 5160) from the recall list, report results or a campaign's audience: one per household, never for someone who asked not to be contacted, moved or died, or has no complete address — those are listed. |
| **Products & gift certificates** | Sell toothbrushes and whitening kits from the ledger or checkout (a charge with sales tax when the practice sets a rate; stock comes off Supplies). Gift certificates: the payment is held for the certificate (not income, not the buyer's credit) until it is used against a balance, in parts; balance lookup by code, printable certificate, outstanding report and CSV, expiry as a setting (never, by default; at least five years when set). Everything is voided with reversing entries, never edited or deleted. |
...
| PDMP (prescription monitoring) | `PDMP=gateway` with `PDMP_URL`, `PDMP_CLIENT_ID`, `PDMP_SECRET`, `PDMP_STATE` and optionally `PDMP_NAME` (a PMP gateway service such as Bamboo Health's; the adapter is `server/src/pdmp.js`). `PDMP=sandbox` simulates answers (the default outside production); `PDMP=none` (the default in production) means prescribers record what the state website showed. Calls appear in Connection activity without patient details. |
...
| AI features | `ANTHROPIC_API_KEY` turns on the assistant, scribe, benefit and EOB reading, claim narratives and appeals, Ask your data, the AI receptionist, call summaries and review replies. X-ray reading (FDA-cleared vendors only, each with a contract and BAA): `XRAY_AI=pearl` / `overjet` / `videahealth` with `XRAY_AI_KEY` (and `XRAY_AI_URL`), or `XRAY_AI=sandbox` for demos (`XRAY_AI_SANDBOX_VENDOR`). A general-purpose model is not used to read x-rays. No-show and denial predictions use the office's own history by default (no key needed); `PREDICT_DRIVER=jev` swaps in TypeSafe AI's Jev (`JEV_MODE=sandbox`, or live with `JEV_API_KEY`, `JEV_URL` and a signed BAA: `JEV_BAA=signed`) — see `docs/predictions.md`. |
...
In **Settings → Import from another system**, pick the old system and follow the steps: **choose → upload → check (dry run) → import → reconciliation**.

- **Open Dental** — upload the database backup (`.sql`; read in the browser, only the columns needed are sent). The full ledger history comes across and each family ends at exactly Open Dental's balance.
- **Dentrix, Eaglesoft, Curve** — upload a `.zip` of the system's export (Dentrix Office Manager / Data Extract lists, Patterson's Eaglesoft data export, Curve Hero's JSON or CSV export). The zip is opened in the browser and only the columns we use are sent (never Social Security numbers). Providers, chairs, patients and families (guarantors), insurance carriers/plans/policies, appointments, completed and planned treatment (history without charges), recall, clinical notes (signed) and perio charts come across; each family gets one **balance forward** equal to what the old system showed — ledger history is never re-posted.

The dry run shows, per kind of record, how many are new, updates, left out (and why) and problems, the accounts receivable total, and every value that couldn't be matched (provider IDs, statuses, non-CDT procedure codes, ledger transaction types) with a choice for each. After the import a reconciliation table compares the source against what was brought in, checked against the database, and the A/R against the balance forward posted; anything left out also becomes a Needs attention item. Each record remembers its ID in the old system, so importing a corrected export again updates what's there instead of duplicating it. Conversions run as the `import` actor in the audit log. See [docs/conversions.md](docs/conversions.md) for what each converter reads and how it maps.

Single lists can also be brought in as CSV (Open Dental, Dentrix, Eaglesoft, Curve or any spreadsheet): import patients first (with families and, optionally, balances), then insurance, balances, appointments, recalls and treatment. Columns are matched by name and can be changed, and a dry run shows what the first rows would do before anything is saved. An import can be undone until its records have been used.
...
1. In **Settings → Imaging bridges**, an administrator presses **Set up a workstation**, names it, picks its imaging programs from the preset list (DEXIS, Sidexis, CS Imaging, Romexis, XrayVision, VixWin, Schick CDR, Eaglesoft/Patterson Imaging, EzDent-i, Progeny and about 10 more; see `bridge/presets.json`) and an optional sensor, then downloads the install package. The zip holds the bridge, `presets.json`, the filled-in `bridge-config.json` and installers. It carries the workstation's key, which is shown only once, so it can only be downloaded right after the workstation is added (within 30 minutes). Details: [docs/imaging-bridges.md](docs/imaging-bridges.md).
2. On that PC, unzip the package and double-click `install.cmd` (on macOS/Linux, `sh install.sh`). It installs Node.js if it's missing (winget, else the official nodejs.org installer), copies the bridge to `C:\DentalMachine`, registers a logon task that runs the bridge as the signed-in person and restarts it if it stops (not a Windows service, which couldn't open imaging programs on the operatory screen), and runs the setup check. `uninstall.cmd` removes it.
3. Point each imaging program's export at the folder listed in `SETUP.txt`. Presets marked `"verify"` in `presets.json` use command-line switches that haven't been confirmed for every vendor version: open a test patient once and correct `args`/`writeFile` in `bridge-config.json` if needed. Presets can be overridden field by field (`"apps": [{ "preset": "dexis", "command": "D:\\DEXIS\\DEXIS.exe" }]`), and apps written out in full (see `bridge/bridge-config.example.json`) still work. Programs that read the patient from a file are supported through `writeFile`.
4. To set it up by hand instead: save the bridge program, `presets.json` and the wizard's **bridge-config.json only** file in one folder, install Node.js 18 or newer, and run `node dental-machine-bridge.mjs bridge-config.json`, for example as a startup task.
...
6. Check the setup with `node dental-machine-bridge.mjs bridge-config.json --check` (programs, export folders, sensor, the state file). While running, the bridge re-checks itself every 10 minutes and reports to **Settings → Imaging bridges**; any problem (a moved export folder, an unplugged sensor, uploads failing) becomes a **Needs attention** item that clears itself once fixed.
...
| Compliance log | `GET/POST /incidents`, `PUT /incidents/:id`, `POST /incidents/:id/resolve\|reopen\|void`, `GET /incidents/report` (`?format=csv`), `GET/POST /exposures`, `PUT /exposures/:id`, `POST /exposures/:id/close\|void`, `GET /exposures/export.csv`, `GET/POST /patients/:id/disclosures`, `POST /disclosures/:id/void`, `GET /disclosures`, `GET /patients/:id/disclosures/accounting` (`?format=csv`), `GET /patients/:id/record-export?recipient=&purpose=` |
| Letters & labels | `GET/POST /letter-templates`, `PUT /letter-templates/:id`, `POST /patients/:id/letters/preview`, `GET/POST /patients/:id/letters`, `GET /letters/:id/pdf`, `POST /letters/batch`, `POST /mailing-labels` (`patient_ids`, or a campaign `segment`) |
| Products & gift certificates | `GET/PUT /retail/settings`, `GET/POST /retail/products`, `PUT /retail/products/:id`, `GET/POST /patients/:id/retail-sales`, `POST /retail-sales/:id/void`, `GET/POST /gift-certificates`, `GET /gift-certificates/lookup?code`, `GET /gift-certificates/:id` (`/certificate.pdf`), `POST /patients/:id/gift-certificates/redeem`, `POST /gift-certificates/redemptions/:entryId/void`, `POST /gift-certificates/:id/void` |
| PDMP | `GET /pdmp`, `GET/POST /patients/:id/pdmp-checks`; `POST /patients/:id/prescriptions` takes `pdmp_check_id` or `pdmp_override_reason` for a controlled substance |
...
- Auto-update for the imaging bridge (the installer and setup wizard exist; updates are still a re-download)

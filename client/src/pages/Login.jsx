...
import { useAuth, takeSsoError, takeResetToken } from '../auth.jsx';
  ...
  return (
    <div className="auth-page">
      <div className="card auth-card">
        ...
        <div className="muted">{{ login: 'Sign in to your practice', register: 'Create your practice account', forgot: 'Reset your password', reset: 'Choose a new password' }[mode]}</div>
        <form onSubmit={(e) => { e.preventDefault(); submit(); }}>
          ...
          {['forgot', 'reset'].includes(mode) ? null : sso?.required ? <p className="muted" style={{ fontSize: 13, margin: 0 }}>Your practice signs in with {sso.name}.</p> : (
            <label>
              Password
          ...
          {(!sso?.required || mode !== 'login') && (
            <button className={sso && mode === 'login' ? '' : 'primary'} disabled={busy} style={{ justifyContent: 'center' }}>
              {busy ? 'Please wait…' : { login: sso ? 'Sign in with password' : 'Sign in', register: 'Create practice', forgot: 'Email me a reset link', reset: 'Set new password' }[mode]}
            </button>
          )}
        </form>

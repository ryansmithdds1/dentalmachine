...
export default function Statement() {
  const { id } = useParams();
  const { data: s } = useApi(`/patients/${id}/statement`);
    ...
    <>
      ...
      <div className="card" style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <table>
          ...
          <tbody>
            {s.entries.map((e) => (
              <tr key={e.id}><td>{fmtDate(e.entry_date)}</td><td>{e.description}</td><td>{label(e.type)}</td><td className="num">{money(e.amount)}</td></tr>
            ))}
            <tr className="totals-row"><td colSpan={3}>Amount due</td><td className="num">{money(s.balance)}</td></tr>
          </tbody>
        </table>

...
import ShowMeBrowser from '../components/tours/ShowMeBrowser.jsx';
...
// Three parts: "How do I…" — the user manual, one page per office task with steps and screenshots
// (components/manual, generated by `npm run manual`); "Show me" — the same tasks as guided walkthroughs you do
// yourself on a practice patient (components/tours, `npm run tours`); and the guides below, a paragraph per feature.
export default function Help() {
  const [params, setParams] = useSearchParams();
  const tab = ['guides', 'showme'].includes(params.get('tab')) ? params.get('tab') : 'how';
  ...
  return (
    <>
      ...
      <div className="tabs no-print" role="tablist">
        ...
        <button type="button" role="tab" aria-selected={tab === 'showme'} className={tab === 'showme' ? 'active' : ''} onClick={() => set({ tab: 'showme' })}>Show me</button>
        <button type="button" role="tab" aria-selected={tab === 'guides'} className={tab === 'guides' ? 'active' : ''} onClick={() => set({ tab: 'guides' })}>Guides</button>
      </div>
      {tab === 'how' && <ManualBrowser how={params.get('how')} q={params.get('q') || ''} print={params.get('print') === '1'} setParams={set} />}
      {tab === 'showme' && <ShowMeBrowser q={params.get('q') || ''} setQ={(q) => set({ tab: 'showme', q }, { replace: true })} />}
      {tab === 'guides' && <Guides />}
    </>
  );
}

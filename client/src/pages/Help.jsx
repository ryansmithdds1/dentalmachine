...
const ARTICLES = [
  ...
  A('No-show and denial predictions', '/reports?tab=predictions', 'Each upcoming visit shows its no-show risk (a percentage and why) on the hover card and visit panel, and on the card when it’s higher than usual; claims show how likely the payer is to deny them, from this office’s own history. They only inform — nothing is cancelled or held. Reports → Prediction accuracy shows how well they’ve matched what happened.', 'no-show risk prediction denial likelihood accuracy'),
...
export default function Help() {
  ...
  return (
    <>
      <div className="page-header"><div><h1>Help</h1><div className="muted">How to do things in Dental Machine.</div></div></div>
      <input type="search" placeholder="Search help…" value={q} onChange={(e) => setQ(e.target.value)} aria-label="Search help" style={{ width: '100%', marginBottom: 12 }} />
      {shown.map((a) => (

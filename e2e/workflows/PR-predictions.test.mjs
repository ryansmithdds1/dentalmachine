...
// and Billing → Ready to approve shows each claim's denial percentage, with the reasons in plain words. Predictions
// only inform — nothing is cancelled, moved or held. Screens in light and dark are saved to SHOTS_DIR
// (default <tmp>/dm-shots) for a person to look at.
...
let app; let browser; let s; let DAY; let doc; let chair; let risky; let visit; let target;
...
before(async () => {
  ...
  const hist = await s.post('/patients', { first_name: 'Hal', last_name: 'History', dob: '1970-01-01' });
  const hp = await s.post(`/patients/${hist.id}/insurance`, { carrier_id: carrier.id, subscriber_name: 'Hal History', subscriber_id: 'PH1', group_number: 'G1' });
...
test('Ready to approve: each claim shows its denial percentage with the payer’s history', async () => {
  const { page } = s;
  ...
  assert.match(await page.textContent('.wl-detail .denial-lines'), /Predict Dental has denied 4 of 4 D2950/);
...
test('Reports → Prediction accuracy shows predicted next to what happened', async () => {
  const { page } = s;
  await page.goto(`${app.base}/reports?tab=predictions`);
  await page.waitForSelector('.prediction-accuracy');
  await page.waitForFunction(() => !document.querySelector('.prediction-accuracy')?.textContent.includes('Working it out'));
...
test('screens in dark mode: schedule hover and panel, Ready to approve', async () => {
  ...
  await row.waitFor();
  await row.click();
  await d.page.waitForSelector('.wl-detail .denial-lines');

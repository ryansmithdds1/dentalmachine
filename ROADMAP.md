...
3. ✅ **P2** Portal: download statements and receipts; update insurance with card photos. _(Done: statements and receipts download as PDFs; a new Insurance section shows each family member's coverage and lets them send new insurance — company, member ID, group, policyholder — with photos of the front and back of the card taken on the phone; the photos are filed in the chart as insurance cards, the office gets a task, and the insurance tab shows the update with the photos until someone enters it and marks it done.)_
...
3. ✅ **P2** Load test with a 50,000-patient database; add indexes; add pagination everywhere. _(Done: `npm run loadtest` builds a 50,000-patient practice (129k visits, 255k procedures, 485k ledger entries, 69k claims) and times 27 screens and reports against a budget; CI runs it at 5,000 patients. It found and fixed: 29 missing indexes (patient search 2 s → 14 ms), collections 22 s → under 1 s (per-account queries replaced by grouped ones), A/R aging rewritten as one pass over the ledger, and the claims worklist paging paid claims straight from the database. Claims, recalls, payment plans, collections, aging and outstanding-claims lists now come a page at a time with “Show more” and the full count; CSV exports still include everything. On Postgres at 50,000 patients every screen is under 0.8 s and every report under 1 s.)_
4. ✅ **P2** Keyboard shortcuts help sheet (`?`). _(Done: `?` (or “Keyboard shortcuts” under the user menu) shows every shortcut — search, schedule, charting and perio, messages — and new “G then a letter” jumps (G S schedule, G P patients, G B billing, G R reports…); shortcuts never fire while typing in a box.)_

...
| **AI features** (scribe, benefit and EOB reading, claim narratives and appeals, Ask your data, the AI receptionist, call summaries, review replies) | `ANTHROPIC_API_KEY` | The facts for the one task: a visit's conversation and chart summary, an x-ray, a benefit summary or EOB, a claim's chart facts, the numbers a question needs, a phone conversation | Covered by the same Anthropic BAA as the assistant. The scribe's conversation isn't stored. |
| **X-ray AI vendor** (Pearl Second Opinion, Overjet, VideaHealth) | `XRAY_AI=pearl` / `overjet` / `videahealth`, `XRAY_AI_KEY`, `XRAY_AI_URL` | X-ray images only (with a random per-read reference — no name, birth date or chart ids) | FDA-cleared detection. Each needs a signed contract **and BAA before any real image is sent**; until then use `XRAY_AI=sandbox`. A general-purpose model is not used to read x-rays. |
| **Prediction vendor** (TypeSafe AI's Jev) — off by default | `PREDICT_DRIVER=jev`, `JEV_MODE=live`, `JEV_API_KEY`, `JEV_URL`, `JEV_BAA=signed` | De-identified features for no-show and denial predictions: counts, rates and categories with an opaque per-request reference — no names, birth dates, contact details, record ids or dates of service | Still derived from patient records, so **sign a BAA before live mode**; live mode refuses to send without `JEV_BAA=signed`. Until then the built-in model (no outside calls) or `JEV_MODE=sandbox`. See `docs/predictions.md`. |
...
| **Speech-to-text** (Deepgram: call transcripts and note dictation) | `TRANSCRIBE=deepgram`, `DEEPGRAM_API_KEY` | Call recordings, and what the dentist dictates into a note | Deepgram signs BAAs on its enterprise plans; get one before transcribing real calls or dictation. With it set, dictation uses Deepgram instead of the browser's own recognition. |

...
import { requireFields, insert, findOr404, audit, practiceNow } from '../util.js';
...
export default function frontDeskRoutes({ db }) {
  ...
  r.get('/huddle', requirePermission('schedule:read'), (req, res) => {
    ...
    const practice = db.get('SELECT daily_goal FROM practices WHERE id = ?', pid);
    const appts = db.all(
    ...
    const rows = appts.map((a) => {
      const balance = db.get('SELECT COALESCE(SUM(amount),0) AS n FROM ledger_entries WHERE patient_id = ?', a.patient_id).n;
      const gid = a.guarantor_id || a.patient_id;
      const familyBalance = db.get('SELECT COALESCE(SUM(l.amount),0) AS n FROM ledger_entries l JOIN patients p ON p.id = l.patient_id WHERE p.id = ? OR p.guarantor_id = ?', gid, gid).n;
      const unscheduled = db.get(
      ...
      const recall = db.get("SELECT due_date, type FROM recalls WHERE patient_id = ? AND status IN ('due','contacted') ORDER BY due_date LIMIT 1", a.patient_id);
      const policy = primaryPolicy(db, pid, a.patient_id);
      const lastElig = policy ? db.get('SELECT status, created_at FROM eligibility_checks WHERE patient_insurance_id = ? ORDER BY id DESC LIMIT 1', policy.id) : null;
      const formsPending = db.get("SELECT COUNT(*) AS n FROM form_requests WHERE patient_id = ? AND status = 'pending'", a.patient_id).n;
      const labs = db.all("SELECT description, status, due_date FROM lab_cases WHERE patient_id = ? AND status IN ('sent','returned_for_adjustment')", a.patient_id);
      const flags = [];
      if (a.patient_since.slice(0, 10) >= addDays(date, -30) || !db.get("SELECT 1 FROM procedures WHERE patient_id = ? AND status = 'completed' LIMIT 1", a.patient_id)) flags.push('new_patient');
  ...
  r.get('/appointments/:aid/route-slip', requirePermission('schedule:read'), (req, res) => {
    const pid = req.user.practice_id;
    const a = findOr404(db, 'appointments', req.params.aid, pid, 'Appointment');
    const patient = db.get('SELECT * FROM patients WHERE id = ?', a.patient_id);
    audit(db, req, 'route_slip.print', 'appointments', a.id);
    res.json({
      appointment: db.get(
      ...
      patient,
      practice: db.get('SELECT name, phone FROM practices WHERE id = ?', pid),
      guarantor: patient.guarantor_id ? db.get('SELECT id, first_name, last_name FROM patients WHERE id = ?', patient.guarantor_id) : null,
      policy: primaryPolicy(db, pid, patient.id) || null,
      balance: db.get('SELECT COALESCE(SUM(amount),0) AS n FROM ledger_entries WHERE patient_id = ?', patient.id).n,
      todays_procedures: db.all("SELECT code, description, tooth, surfaces, fee FROM procedures WHERE appointment_id = ? AND status != 'cancelled'", a.id),
      unscheduled: db.all("SELECT code, description, tooth, surfaces, fee FROM procedures WHERE patient_id = ? AND status = 'planned' AND appointment_id IS NULL ORDER BY priority", patient.id),
      recall: db.all("SELECT type, due_date FROM recalls WHERE patient_id = ? AND status != 'inactive'", patient.id),
      last_visit: db.get("SELECT MAX(start_time) AS t FROM appointments WHERE patient_id = ? AND status = 'completed' AND start_time < ?", patient.id, a.start_time).t,
      last_note: db.get('SELECT body, created_at FROM clinical_notes WHERE patient_id = ? ORDER BY id DESC LIMIT 1', patient.id) || null,
    });
  });

  // ---- Follow-up lists (the "unscheduled" and "broken appointment" lists) ----
  const lastContact = (patientId, kind) => db.get('SELECT outcome, note, created_at FROM followups WHERE patient_id = ? AND kind = ? ORDER BY id DESC LIMIT 1', patientId, kind) || null;

  r.get('/followups/unscheduled', requirePermission('schedule:read'), (req, res) => {
    const pid = req.user.practice_id;
    const rows = db.all(
      `SELECT p.id AS patient_id, p.first_name, p.last_name, p.phone, p.email, COUNT(pr.id) AS procedures, SUM(pr.fee) AS amount,
         MIN(COALESCE(tp.accepted_at, tp.created_at)) AS planned_since, MAX(tp.status = 'accepted') AS accepted,
       ...
       GROUP BY p.id ORDER BY accepted DESC, amount DESC`, pid, practiceNow(db, pid),
    ).map((x) => ({ ...x, last_contact: lastContact(x.patient_id, 'unscheduled') }));
    res.json(rows);
  });

  r.get('/followups/broken', requirePermission('schedule:read'), (req, res) => {
    const pid = req.user.practice_id;
    ...
    ).map((x) => ({ ...x, last_contact: lastContact(x.patient_id, 'broken') }));
    res.json(rows);
  });

  r.get('/patients/:id/followups', requirePermission('patients:read'), (req, res) => {
  ...
  r.post('/patients/:id/followups', requirePermission('patients:write'), (req, res) => {
    const patient = findOr404(db, 'patients', req.params.id, req.user.practice_id, 'Patient');
    ...
    const id = insert(db, 'followups', { practice_id: req.user.practice_id, patient_id: patient.id, kind, outcome, note: note ? String(note).slice(0, 1000) : null, created_by: req.user.id });
    audit(db, req, 'followup.create', 'followups', id, { kind, outcome });
    res.status(201).json(db.get('SELECT * FROM followups WHERE id = ?', id));
  });

  r.post('/patients/:id/medical-reviewed', requirePermission('clinical:write'), (req, res) => {
    const patient = findOr404(db, 'patients', req.params.id, req.user.practice_id, 'Patient');
    db.run("UPDATE patients SET medical_reviewed_at = datetime('now') WHERE id = ?", patient.id);
    audit(db, req, 'patient.medical_reviewed', 'patients', patient.id);
    res.json({ ok: true });
  });

  // Global quick search (command palette): patients by name/phone/DOB/ID, plus claims by number.
  r.get('/search', requirePermission('patients:read'), (req, res) => {
    ...
    const claims = /^#?\d+$/.test(q)
      ? db.all('SELECT c.id, c.status, p.first_name, p.last_name FROM claims c JOIN patients p ON p.id = c.patient_id WHERE c.practice_id = ? AND c.id = ?', pid, Number(q.replace('#', '')))
      : [];

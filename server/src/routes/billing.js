...
export default function billingRoutes({ db }) {
  ...
  r.post('/patients/:id/payments', requirePermission('billing:write'), (req, res) => {
    ...
    const plan = row.payment_plan_id ? findOr404(db, 'payment_plans', row.payment_plan_id, req.user.practice_id, 'Payment plan') : null;
    if (plan) {
      const family = db.get('SELECT 1 AS ok FROM patients WHERE id = ? AND (id = ? OR guarantor_id = ?)', patient.id, plan.patient_id, plan.patient_id);
    ...
    if (plan && planStatus(db, plan, '9999-12-31').remaining === 0) db.run("UPDATE payment_plans SET status = 'completed' WHERE id = ?", plan.id);
  ...
  r.get('/patients/:id/statement', requirePermission('billing:read'), (req, res) => {
    ...
    const addressee = family && patient.guarantor_id ? db.get('SELECT * FROM patients WHERE id = ?', patient.guarantor_id) : patient;
    const ids = family
      ? db.all('SELECT id FROM patients WHERE practice_id = ? AND (id = ? OR guarantor_id = ?)', req.user.practice_id, addressee.id, addressee.id).map((x) => x.id)
      : [patient.id];
    const inList = ids.map(() => '?').join(',');
    const prior = db.get(`SELECT COALESCE(SUM(amount),0) AS n FROM ledger_entries WHERE practice_id = ? AND patient_id IN (${inList}) AND entry_date < ?`, req.user.practice_id, ...ids, since).n;
    const entries = db.all(
    ...
    const balance = db.get(`SELECT COALESCE(SUM(amount),0) AS n FROM ledger_entries WHERE practice_id = ? AND patient_id IN (${inList})`, req.user.practice_id, ...ids).n;
    audit(db, req, 'statement.generate', 'patients', patient.id, { family });

...
export default function documentRoutes({ db, storage }) {
  ...
  r.get('/patients/:id/documents', requirePermission('clinical:read'), (req, res) => {
  ...
  r.post(
    ...
    (req, res) => {
      ...
      const { storageKey, encrypted } = storage.save(req.user.practice_id, req.body);
      const id = insert(db, 'documents', {
      ...
      audit(db, req, 'document.upload', 'documents', id, { patient_id: patient.id, category });
      res.status(201).json(db.get('SELECT id, category, filename, mime, size, tooth, notes, created_at FROM documents WHERE id = ?', id));
    },
  );

  r.get('/documents/:did/file', requirePermission('clinical:read'), (req, res) => {
    const doc = findOr404(db, 'documents', req.params.did, req.user.practice_id, 'Document');
    if (doc.deleted_at) throw new HttpError(404, 'Document not found');
    const data = storage.read(doc.storage_key, !!doc.encrypted);
    if (!data) throw new HttpError(404, 'File missing from storage');
    audit(db, req, 'document.view', 'documents', doc.id, { patient_id: doc.patient_id });
    res.set({
  ...
  r.delete('/documents/:did', requirePermission('clinical:write'), (req, res) => {
    const doc = findOr404(db, 'documents', req.params.did, req.user.practice_id, 'Document');
    db.run("UPDATE documents SET deleted_at = datetime('now') WHERE id = ?", doc.id);
    audit(db, req, 'document.delete', 'documents', doc.id, { patient_id: doc.patient_id });
    res.json({ ok: true });
  });

...
export function findBlockouts(db, practiceId, { start_time, end_time, provider_id, operatory_id }) {
  return db.all(
...
export function validateAppt(db, practiceId, row, { overrideBlockout = false } = {}) {
  ...
  if (row.appointment_type_id) findOr404(db, 'appointment_types', row.appointment_type_id, practiceId, 'Appointment type');
  ...
  if (!overrideBlockout) {
    const blocks = findBlockouts(db, practiceId, row);
...
export function openSlots(db, practiceId, providerId, date, { duration = 60, step = 10, after = null, open = null, close = null } = {}) {
  const practice = db.get('SELECT office_hours FROM practices WHERE id = ?', practiceId);
...
export default function scheduleRoutes({ db }) {
  ...
  r.post('/appointments', requirePermission('schedule:write'), (req, res) => {
    ...
    const type = row.appointment_type_id ? findOr404(db, 'appointment_types', row.appointment_type_id, req.user.practice_id, 'Appointment type') : null;
    ...
    validateAppt(db, req.user.practice_id, row, { overrideBlockout: !!req.body.override_blockout });
    const id = db.tx(() => {
      const newId = insert(db, 'appointments', { ...row, practice_id: req.user.practice_id });
      ...
      if (type?.procedure_codes && req.body.add_type_procedures !== false && !(req.body.procedure_ids || []).length) {
        for (const code of JSON.parse(type.procedure_codes)) {
          const pc = db.get('SELECT * FROM procedure_codes WHERE practice_id = ? AND code = ? AND active = 1', req.user.practice_id, code);
  ...
  r.put('/appointments/:id', requirePermission('schedule:write'), (req, res) => {
    ...
    if (!['cancelled', 'no_show'].includes(merged.status)) validateAppt(db, req.user.practice_id, merged, { overrideBlockout: !!req.body.override_blockout });
    ...
    if (Number(row.provider_id) !== existing.provider_id) db.run("UPDATE procedures SET provider_id = ? WHERE appointment_id = ? AND status = 'planned'", row.provider_id, existing.id);
  ...
  r.patch('/appointments/:id/status', requirePermission('schedule:write'), (req, res) => {
    ...
    if (['cancelled', 'no_show'].includes(existing.status) && !['cancelled', 'no_show'].includes(status)) {
      validateAppt(db, req.user.practice_id, { ...existing, status }, { overrideBlockout: true });
    }
    db.run(
  ...
  r.get('/availability', requirePermission('schedule:read'), (req, res) => {
    ...
    const slots = openSlots(db, pid, Number(provider_id), date, { duration, open: req.query.open, close: req.query.close });
  ...
  r.get('/schedule', requirePermission('schedule:read'), (req, res) => {
    const pid = req.user.practice_id;
    const from = req.query.from || practiceNow(db, pid).slice(0, 10);
    const to = req.query.to || from;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(from) || !/^\d{4}-\d{2}-\d{2}$/.test(to) || to < from) throw new HttpError(400, 'from/to must be YYYY-MM-DD');
    const practice = db.get('SELECT office_hours, daily_goal FROM practices WHERE id = ?', pid);
    ...
    res.json({
      ...
      appointments,
      blockouts: db.all('SELECT * FROM blockouts WHERE practice_id = ? AND start_time < ? AND end_time > ? ORDER BY start_time', pid, `${to} 24:00`, `${from} 00:00`),
      production,
    });
  });

  // ASAP list: booked patients who'd take an earlier opening.
  r.get('/asap', requirePermission('schedule:read'), (req, res) => {
    const pid = req.user.practice_id;
    res.json(db.all(`${SELECT} WHERE a.practice_id = ? AND a.asap = 1 AND a.start_time > ? AND a.status IN ('scheduled','confirmed') ORDER BY a.start_time`, pid, practiceNow(db, pid)));
  });

  // ---- Blockouts (lunch, meetings, holidays, "crown seats only"…) ----
  const BLOCK_FIELDS = ['provider_id', 'operatory_id', 'start_time', 'end_time', 'reason'];
  const validateBlockout = (req, row) => {
    row.start_time = normalizeDateTime(row.start_time, 'start_time');
    row.end_time = normalizeDateTime(row.end_time, 'end_time');
    if (row.end_time <= row.start_time) throw new HttpError(400, 'end_time must be after start_time');
    if (row.provider_id) findOr404(db, 'providers', row.provider_id, req.user.practice_id, 'Provider');
    if (row.operatory_id) findOr404(db, 'operatories', row.operatory_id, req.user.practice_id, 'Operatory');
  };
  r.get('/blockouts', requirePermission('schedule:read'), (req, res) => {
    const from = req.query.from || '0000-00-00';
    const to = req.query.to || '9999-12-31';
    res.json(db.all('SELECT * FROM blockouts WHERE practice_id = ? AND start_time < ? AND end_time > ? ORDER BY start_time', req.user.practice_id, `${to} 24:00`, `${from} 00:00`));
  });
  r.post('/blockouts', requirePermission('schedule:write'), (req, res) => {
    const row = pick(req.body, BLOCK_FIELDS);
    requireFields(row, ['start_time', 'end_time', 'reason']);
    validateBlockout(req, row);
    // Repeat weekly for N weeks (e.g. lunch every Tuesday).
    const repeat = Math.min(Math.max(Number(req.body.repeat_weeks) || 1, 1), 52);
    const ids = db.tx(() => Array.from({ length: repeat }, (_, i) => {
      const shift = (v) => `${new Date(Date.parse(`${v.slice(0, 10)}T12:00:00Z`) + i * 7 * 86400_000).toISOString().slice(0, 10)} ${v.slice(11)}`;
      return insert(db, 'blockouts', { ...row, start_time: shift(row.start_time), end_time: shift(row.end_time), practice_id: req.user.practice_id, created_by: req.user.id });
    }));
    audit(db, req, 'blockout.create', 'blockouts', ids[0], { count: ids.length });
    changed(req, row.start_time);
    res.status(201).json(db.all(`SELECT * FROM blockouts WHERE id IN (${ids.map(() => '?').join(',')})`, ...ids));
  });
  r.put('/blockouts/:bid', requirePermission('schedule:write'), (req, res) => {
    const existing = findOr404(db, 'blockouts', req.params.bid, req.user.practice_id, 'Blockout');
    const row = { ...pick(existing, BLOCK_FIELDS), ...pick(req.body, BLOCK_FIELDS) };
    validateBlockout(req, row);
    update(db, 'blockouts', existing.id, req.user.practice_id, row);
    audit(db, req, 'blockout.update', 'blockouts', existing.id);
    changed(req, existing.start_time, row.start_time);
    res.json(db.get('SELECT * FROM blockouts WHERE id = ?', existing.id));
  });
  r.delete('/blockouts/:bid', requirePermission('schedule:write'), (req, res) => {
    const existing = findOr404(db, 'blockouts', req.params.bid, req.user.practice_id, 'Blockout');
    db.run('DELETE FROM blockouts WHERE id = ?', existing.id);
    audit(db, req, 'blockout.delete', 'blockouts', existing.id);
    changed(req, existing.start_time);
    res.json({ ok: true });
  });

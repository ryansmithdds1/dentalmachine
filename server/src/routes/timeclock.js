...
export default function timeclockRoutes({ db }) {
  ...
  r.get('/timeclock/reports', async (req, res) => {
    ...
    if (!rates) {
      ...
      const prod = await db.all("SELECT location_id, COALESCE(SUM(amount), 0) AS cents FROM real_ledger_entries ledger_entries WHERE practice_id = ? AND type = 'charge' AND retail_sale_id IS NULL AND voided_at IS NULL AND entry_date >= ? AND entry_date <= ? GROUP BY location_id", req.user.practice_id, from, to);

...
export default function engagementRoutes({ db, messenger, config }) {
  ...
  const practiceName = (pid) => db.get('SELECT name FROM practices WHERE id = ?', pid).name;

  // ---- Messages ----
  r.get('/messaging/status', (_req, res) => res.json({ ...messenger.status, app_url: config.appUrl }));

  r.get('/messages', requirePermission('patients:read'), (req, res) => {
  ...
  r.post('/appointments/:aid/remind', requirePermission('schedule:write'), async (req, res) => {
    const appt = findOr404(db, 'appointments', req.params.aid, req.user.practice_id, 'Appointment');
    ...
    audit(db, req, 'appointment.remind', 'appointments', appt.id);
    res.status(201).json(msg);
  });

  r.post('/recalls/:rid/remind', requirePermission('schedule:write'), async (req, res) => {
    const recall = findOr404(db, 'recalls', req.params.rid, req.user.practice_id, 'Recall');
  ...
  r.post('/messaging/run-reminders', requireAdmin, async (req, res) => {
    ...
    audit(db, req, 'messaging.run_reminders', null, null, { sent });
    res.json({ sent });
  });

  // ---- Online booking queue ----
  r.get('/booking-requests', requirePermission('schedule:read'), (req, res) => {
    ...
    res.json(db.all(
      ...
      matches: db.all(
        `SELECT id, first_name, last_name, dob, phone FROM patients WHERE practice_id = ? AND status != 'archived'
         AND ((lower(first_name) = lower(?) AND lower(last_name) = lower(?)) OR (? IS NOT NULL AND dob = ? AND lower(last_name) = lower(?)))
         LIMIT 5`,
  ...
  r.post('/booking-requests/:bid/accept', requirePermission('schedule:write'), async (req, res) => {
    ...
    const b = findOr404(db, 'booking_requests', req.params.bid, pid, 'Booking request');
    ...
    const apptId = db.tx(() => {
      let patientId = req.body?.patient_id ? findOr404(db, 'patients', req.body.patient_id, pid, 'Patient').id : null;
      if (!patientId) {
        patientId = insert(db, 'patients', {
      ...
      db.run("UPDATE booking_requests SET status = 'accepted', patient_id = ?, appointment_id = ?, handled_by = ?, handled_at = datetime('now') WHERE id = ?", patientId, id, req.user.id, b.id);
      return id;
    });
  ...
  r.post('/booking-requests/:bid/decline', requirePermission('schedule:write'), async (req, res) => {
    const b = findOr404(db, 'booking_requests', req.params.bid, req.user.practice_id, 'Booking request');
    if (b.status !== 'pending') throw new HttpError(409, `Request already ${b.status}`);
    db.run("UPDATE booking_requests SET status = 'declined', handled_by = ?, handled_at = datetime('now') WHERE id = ?", req.user.id, b.id);
    audit(db, req, 'booking.decline', 'booking_requests', b.id);
    let message = null;
    if (req.body?.notify !== false) {
      const practice = db.get('SELECT name, phone FROM practices WHERE id = ?', req.user.practice_id);
  ...
  r.post('/patients/:id/form-requests', requirePermission('patients:write'), async (req, res) => {
    ...
    const id = insert(db, 'form_requests', {
    ...
    if (req.body?.send) {
      ...
      if (!target) throw new HttpError(400, 'Patient has no reachable phone or email (or has opted out)');
      const name = practiceName(req.user.practice_id);
      message = await sendMessage(db, messenger, {
    ...
    audit(db, req, 'form_request.create', 'form_requests', id);
    res.status(201).json({ id, url, expires_at: expires, message });
  });

  r.get('/patients/:id/forms', requirePermission('clinical:read'), (req, res) => {
    const patient = patientOr404(req);
    res.json({
      requests: db.all('SELECT id, kind, status, expires_at, completed_at, created_at FROM form_requests WHERE practice_id = ? AND patient_id = ? ORDER BY id DESC', req.user.practice_id, patient.id),
      submissions: db.all('SELECT * FROM patient_forms WHERE practice_id = ? AND patient_id = ? ORDER BY id DESC', req.user.practice_id, patient.id)

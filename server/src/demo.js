// A demo practice with realistic sample data (used by `npm run seed` and hosted trial deployments).
...
export const DEMO_EMAIL = 'admin@demo.dentalmachine.app';
export const DEMO_PASSWORD = 'demo-password-123';

// Creates the demo practice unless it already exists; returns whether it did.
export async function seedDemo(db) {
  if (await db.get('SELECT id FROM users WHERE lower(email) = lower(?)', DEMO_EMAIL)) return false;

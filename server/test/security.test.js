...
test('the app page is served with a content security policy that matches vercel.json', async () => {
  ...
  // Our own scripts only, plus Plaid's bank-connection script and the booking page's bot check: nothing inline.
  assert.match(CSP, /script-src 'self' https:\/\/cdn\.plaid\.com\/link\/v2\/stable\/link-initialize\.js https:\/\/challenges\.cloudflare\.com;/);
